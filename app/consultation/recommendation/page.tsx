...
function RecommendationContent() {
  ...
  return (
    <div className="min-h-screen bg-clinic-cream flex flex-col p-6 md:p-10 bg-pattern overflow-x-hidden">
      ...
      {treatments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-16 max-w-7xl mx-auto w-full pb-32">
          {treatments.map((t) => {
            const cheapestOption = t.treatment_price_options?.sort((a: any, b: any) => a.price - b.price)[0];
            return (
              <div key={t.id} className="glass-card overflow-hidden flex flex-col lg:flex-row animate-fade-in border-white/60">
                {/* 療程海報 */}
                <div className="lg:w-2/5 aspect-[4/3] lg:aspect-auto relative bg-gray-50 flex items-center justify-center p-8 border-r border-white/40">
                ...
                <div className="lg:w-3/5 p-10 md:p-14 flex flex-col">
                  ...
                  <p className="text-gray-500 text-lg leading-relaxed mb-10 italic font-light opacity-80">
                  ...
                  <div className="space-y-6 flex-1">
                    <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                      <Info size={14} className="text-clinic-gold" /> 術前後見證案例 (Before & After)
                    </h4>
                    
                    {t.treatment_cases && t.treatment_cases.length > 0 ? (
                      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
                        {t.treatment_cases.map((c: any) => (
                          <div key={c.id} className="min-w-[280px] md:min-w-[320px] bg-white rounded-3xl p-4 shadow-sm border border-gray-100 snap-start group">
                      ...
                      <div className="p-8 bg-gray-50 rounded-[2rem] border border-dashed text-center text-gray-300 text-sm font-bold italic">
                        目前此療程尚無術前後見證案例
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-10 pt-8 border-t border-gray-100 flex gap-4">
                    <Link href={`/treatments/${t.id}`} className="btn-gold flex-1 py-5 text-lg tracking-widest uppercase font-black shadow-clinic-gold/20">
                      查看療程詳情與完整見證 <ArrowRight size={20} />
                    </Link>
                  </div>
                </div>
              </div>
